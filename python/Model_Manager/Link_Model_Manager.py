...
from python.Traffic_Models.MN_Model import MN_Model_Class
...
class Link_Model_Manager_class(Abstract_Model_Manager_class):
    ...
    def __init__(self, configfile, traffic_model_name, gateway, sim_dt, cost_function_name, cost_function_parameters):
        ...
        if traffic_model_name == "static":
            self.traffic_model = Static_Model_Class(self.otm_api)

        elif traffic_model_name == "mn":
            self.traffic_model = MN_Model_Class(self.otm_api, gateway)
        else:
    ...
    def evaluate(self, demand_assignments, T = None, initial_state = None,vect = True):
         # vect variable indicates whether we use vector based functions or not
